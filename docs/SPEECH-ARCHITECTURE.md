...
| `kokororesident` | `on` or `off` | Startup provider setting. `on` selects the Kokoro server protocol so its loaded model survives between utterances. |
| `warm` | `on` | Runtime provider request. Starts resident Kokoro and waits for its model-ready marker; voice mode sends it on entry. |
