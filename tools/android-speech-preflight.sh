#!/bin/sh
# Combined prerequisite check for the Mac + Android speech recipe (INF-17).
#
...
android_preflight_write_local_properties()
{
	...
	if [ -f "$_dest" ]; then
		...
		{
			echo "# Generated by tools/android-speech-preflight.sh (INF-17). Per-machine; not tracked."
			[ -n "$_kept" ] && printf '%s\n' "$_kept"
