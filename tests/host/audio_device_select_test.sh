...
# 5. A capture reports whether it heard anything. Without a usable
#    microphone this can only reach "silent", which is itself the verdict
#    the selection exists to surface — so both outcomes pass here, and only
#    a missing verdict fails.
run_inferno "bind -a '#A' /dev; echo 'in rate 16000 chans 1 bits 16 enc pcm' > /dev/audioctl; dd -if /dev/audio -of /tmp/audiodev-cap.pcm -bs 16000 -count 8; cat /dev/audiodev" >/dev/null
if grep -q "^capture active" "$log"; then
  ...
  echo "SKIP: the capture device delivered no data in this host session"
else
